...
type ChatScreenScreenNavigationProp = StackNavigationProp<
  ...
  Paths.Chat
>;
type ChatScreenRouteProp = RouteProp<RootStackParamList, Paths.Chat>;
...
const ChatScreen = () => {
  ...
  const isCurrentUserSuperAdmin = auth.superAdmin;
    ...
    try {
      if (isCurrentUserSuperAdmin) {
        await mutateAsync({
          ...
          receiverModel: context,
          receiverId: receiverId,
          itemId: id,
        });
        queryClient.invalidateQueries({
          queryKey: [REACT_QUERY_KEYS.messageQueries.getChat + id, context],
        });
      } else {
  ...
  const renderItem = ({ item }: { item: any }) => {
    ...
    const isMyMessage = auth.superAdmin
      ? item.senderModel === 'SuperAdmin'
      : item.senderModel === 'AuctionHouse'
        ? true
        : false;
